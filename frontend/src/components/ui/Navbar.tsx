...
const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <nav className="backdrop-blur-md bg-black/20 border-b border-white/10 sticky top-0 z-40">
      <div className="flex items-center justify-between p-4 w-full max-w-7xl mx-auto">
        <div className="flex items-center">
          <div className="flex items-center mr-8">
            ...
            <span className="font-bold text-xl text-white">DobSwap</span>
          </div>

          <div className="hidden md:flex space-x-1 bg-white/5 backdrop-blur p-1 rounded-xl">
            {tabs.map((tab) => (
              <button
                key={tab.id}
          ...
          <div className="md:hidden relative">
            <button
              className="flex items-center space-x-1 bg-white/5 backdrop-blur px-3 py-2 rounded-xl text-white"
            ...
            {isDropdownOpen && (
              <div className="absolute top-full left-0 mt-1 w-40 bg-black/40 backdrop-blur-lg rounded-xl shadow-lg py-1 z-10 border border-white/10">
                {tabs.map((tab) => (
                  <button
                    key={tab.id}
